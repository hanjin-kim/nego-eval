...
**여기서 결손은 무엇으로 이루어졌는지 말해주지 않습니다.** 점수는 숫자 하나이고
그것을 움직이는 것은 셋입니다. 기대값이 가장 높은 판매자를 얼마나 안정적으로
고르는가, 실패를 어떻게 합의하는가, 그리고 관계에서 무엇을 얻는가. gemini 의
−66 에는 전혀 다른 두 설명이 똑같이 들어맞습니다.

| | 선택 | 합의 | 관계 | 합계 |
|---|---|---|---|---|
| 한 이야기 | 정확도 0.91, −71 | 기준 정책 수준, 0 | 0 | −66 |
| 다른 이야기 | 완벽, 0 | 주는 대로 받음, −194 | **+128** | −66 |

두 번째에서는 모델이 관계에서 오히려 이득을 보면서 다른 곳에서 그보다 더
까먹고 있습니다. 점수는 둘을 구분하지 못하고, 관계 항의 부호조차 정하지
못합니다. 세 수치는 `scripts/accuracy_to_profit.py`,
`scripts/policy_map.py`, `scripts/noise_taxes_relationship.py` 가 같은 판에서
재생해 얻은 것입니다.

그러므로 이 표를 모델이 기록을 얼마나 잘 쓰는지에 대한 측정으로 읽으면 안
됩니다. 이것은 합을 잽니다. 쪼개려면 라운드마다 선택과 합의를 기록해야 합니다
— 모델이 고른 판매자, 그 시점의 기대값 최적 판매자, 그리고 제시액을 받았는지
되받아쳤는지. 그러면 관계는 부호가 있는 잔차로 남습니다. `run_fullplay4.py` 는
시드별 이익만 남기고 라운드를 버리므로 `data/` 에서는 복원되지 않고, 다시
측정해야 합니다.

**다만 강제되는 것이 하나 있습니다. 정보가 한계는 아니었습니다.** 선택 프롬프트는
매번 가격 옆에 배송률을 인쇄하고, 934 라는 기준은 그 밖의 어떤 것도 참조하지
않습니다 — 기록도 기억도 없이 기대값 셋과 최대값 하나입니다. 일곱 중 여섯이 그
똑같은 견적서에 원장까지 더 받고도 그 아래에 있습니다. 선택과 합의의 몫이
어떻게 갈리든, 부족한 것은 보이는 숫자에 접근하는 능력이 아니라 그것을 실행하는
능력입니다. 그러므로 한 경기가 요구하는 쉰 번 남짓의 비교를 얼마나 안정적으로
해내는가는 이 판이 민감하게 반응하는 축이고, 기록에 관한 어떤 주장과도 따로
보고할 값이 있습니다.

두 줄에 대해서는 그 이상까지 못이 박힙니다. 찍어서 고르면 약 276 을 잃는데
(`scripts/accuracy_to_profit.py`), haiku-4.5 는 −288, deepseek-chat 은 −342 로
찍기보다 더 아래입니다. 따라서 합의 쪽도 함께 새고 있어야 합니다. 나머지 줄에
대해서는 이만큼도 확정되지 않습니다.
...
두 질문이 열려 있고, 둘 다 노력이 모자라서 열려 있는 것이 아닙니다.

**점수를 채널로 쪼개기.** 모델 표는 합을 보고합니다 — 선택 산술, 합의, 그리고
관계가 벌어들인 것. 숫자 하나로는 셋을 가를 수 없으므로, 그 줄들을 모델이
기록을 얼마나 잘 쓰는지에 대한 측정으로 읽으면 안 됩니다. 가르려면 라운드마다
선택과 합의를 기록해야 합니다. 모델이 고른 판매자, 그 시점의 기대값 최적
판매자, 제시액을 받았는지 되받아쳤는지. 일곱 모델을 다시 돌리는 일이고, 12
경기 기준 모델당 약 850 콜이며, 비용은 콜 수가 아니라 추론 토큰이 지배합니다.
`run_fullplay4.py` 는 시드별 이익만 남기므로 `data/` 에서는 복원되지 않습니다.

**이 판을 둘 수 있는 것을 학습시키기.** 판은 `src/nego_eval/rl/` 에 RLVR 용으로
포장돼 있고 세 번의 GRPO 실행이 `deploy/` 에 기록돼 있습니다. 어느 것도 아무것도
개선하지 못했고, 그 이유는 어떤 보상 형태로도 닿지 않는 곳에 있습니다. 선택
정확도가 되는 모델들 — 함의 정확도 0.9 이상 — 은 가중치가 닫혀 있고, 미세조정할
수 있는 모델들은 한 판매자 이름만 계속 대는 것과 같거나 그보다 낮은 점수를
받습니다. 산술을 사주는 것은 추론이고, 추론을 켜 두면 토큰이 약 70 배입니다.

추론 크레딧이 있거나, `scripts/thinking_probe.py` 에서 선택 정확도 0.80 정도를
넘기는 오픈 웨이트에 접근할 수 있거나, 이 판으로 다른 것을 해보고 싶다면 이슈를
열어 주세요. 그편이 편하다면 커밋 로그에 주소가 있습니다.

"""The models, on the board that passed the contract.
...
P = json.load(open('data_probe3_positions.json'))[:120]
...
MODELS = ['deepseek/deepseek-chat', 'anthropic/claude-haiku-4.5',
          ...
          'anthropic/claude-opus-5', 'anthropic/claude-fable-5', 'x-ai/grok-4.6']
...
def run(model):
    ...
    with lock:
        json.dump(dict(r, rows=rows), open(f"data_p3_{model.split('/')[-1]}.json", 'w'), indent=1)
...
print(f"3번 판 · 위치 {len(P)}개 · 우연 0.33 · 천장 1.00")
print(f"기준선: 호가판만 0.75 · 부담률만 0.64 · 관측 실패율 0.55 · 최저가 0.02\n")

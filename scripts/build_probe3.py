"""Frozen positions on the board where quality is published and conduct is not.
...
from carryover.game.table3 import VALUE, cast_for, expected, price_of
...
positions = []
for s in range(N):
    make, key, ev, board = cast_for(s, loss=LOSS)
    prior = []
...
json.dump(positions, open('data_probe3_positions.json', 'w'), indent=1)
...
json.dump(rep['heuristics'] | {'recoverable': rep['recoverable']},
          open('data_probe3_baselines.json', 'w'), indent=1)
